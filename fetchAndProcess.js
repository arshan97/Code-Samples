...
async function fetchAndProcess() {
  try {
    const ncepBody = {
      ...
      attributes: [
        "hostname", "mgmtIP", "hwType", "model", "os", "version", "location",
        "tower", "site", "zone", "status", "region", "serial", "rack", "function",
        "role", "organisation"
      ],
    };
    ...
    for (const device of deviceList) {
      const {
        hostname, hwType, tower, organisation
      } = device;
      ...
      try {
        ...
        if (hits && hits.length > 0) {
          ...
          if (expectedTag && elkTag === expectedTag) {
            // Tag match: calculate from timestamp
        ...
        // No hits: use or update from state
        const existing = ageState[hostname];
        if (existing) {
          const lastSeen = existing.last_seen;
          age = existing.age;
          if (lastSeen < today) age += 1;
        } else {
          age = 30;
        }
      ...
      // Add to detail with full device + age info
    ...
    console.log(`✅ Wrote latest output to data.json and updated age_state.json`);
  } catch (err) {
